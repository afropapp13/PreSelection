...
//        chPOT.Merge(f,1000);
	...
	for (int i = 0;i<2500;i++) { if (i%100) { cout << "Event " << i << endl; } ch.Add(Form(pathIn+"_%s/neutrinoselection_filt_*.root",std::to_string(i).c_str())); }
	...
	std::cout << OutName << " # events = " << ch.GetEntries() << std::endl;
