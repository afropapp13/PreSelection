...
void TwoEfficiencyFunction(TString Run, TString XVar, TString YVar , TTree* t, TTree* tTrue, int XBins, double Xmin, double Xmax, int YBins, double Ymin, double Ymax) {
	...
	TString WithQCQual = qualifier + " && NuScore > " + TString(std::to_string(MinimumNuScore)) + " && CandidateP_LLR_PID > "\
			 +  TString(std::to_string(ProtonLLRPIDScore));

	TString WithoutQCQual = qualifierNoExitMuQC + " && NuScore > " + TString(std::to_string(MinimumNuScore)) + " && CandidateP_LLR_PID > "\
			 +  TString(std::to_string(ProtonLLRPIDScore));
...
void ProtonPrintCanvas(TString Run, TString Var, TTree* t, TString Slice) {
	...
	if (Slice == "LowPT") { Label = "0 < #deltap_{T} < 0.2 [GeV/c]"; Cut = Var + " > 0 && " + Var + " < 0.2"; }
	if (Slice == "MidPT") { Label = "0.2 < #deltap_{T} < 0.5 [GeV/c]"; Cut = Var + " > 0.2 && " + Var + " < 0.5"; }
	if (Slice == "HighPT") { Label = "0.5 < #deltap_{T} < 0.75 [GeV/c]"; Cut = Var + " > 0.5 && " + Var + " < " + TString(std::to_string(ArrayNBinsDeltaPT[NBinsDeltaPT])); }
	...
	TString Qualifier = qualifierNoExitMuQC + " && NuScore > " + TString(std::to_string(MinimumNuScore)) + " && CandidateP_LLR_PID > "\
			 +  TString(std::to_string(ProtonLLRPIDScore));
...
/*
		...
		MuonPrintCanvas(Runs[WhichRun],"Reco_Pt"+Recalibrate,tree,"HighPT");

		MuonPrintCanvas(Runs[WhichRun],"Reco_DeltaAlphaT"+Recalibrate,tree,"AllDeltaAlphaT");
		MuonPrintCanvas(Runs[WhichRun],"Reco_DeltaAlphaT"+Recalibrate,tree,"LowDeltaAlphaT");
		MuonPrintCanvas(Runs[WhichRun],"Reco_DeltaAlphaT"+Recalibrate,tree,"MidDeltaAlphaT");
		MuonPrintCanvas(Runs[WhichRun],"Reco_DeltaAlphaT"+Recalibrate,tree,"HighDeltaAlphaT");

		MuonPrintCanvas(Runs[WhichRun],"Reco_DeltaPhiT"+Recalibrate,tree,"AllDeltaPhiT");
...
/*
		...
		ProtonPrintCanvas(Runs[WhichRun],"Reco_Pt"+Recalibrate,tree,"HighPT");

		ProtonPrintCanvas(Runs[WhichRun],"Reco_DeltaAlphaT"+Recalibrate,tree,"AllDeltaAlphaT");
		ProtonPrintCanvas(Runs[WhichRun],"Reco_DeltaAlphaT"+Recalibrate,tree,"LowDeltaAlphaT");
		ProtonPrintCanvas(Runs[WhichRun],"Reco_DeltaAlphaT"+Recalibrate,tree,"MidDeltaAlphaT");
		ProtonPrintCanvas(Runs[WhichRun],"Reco_DeltaAlphaT"+Recalibrate,tree,"HighDeltaAlphaT");

		ProtonPrintCanvas(Runs[WhichRun],"Reco_DeltaPhiT"+Recalibrate,tree,"AllDeltaPhiT");
